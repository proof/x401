...
- "Relayed Delivery to a Remote Handler" subsection for the case where an intermediary (e.g. an MCP tool) hands the request to a remote web wallet/service that does not invoke the Digital Credentials API. The intermediary appends an `https` `return_uri` (a new OPTIONAL, intermediary-supplied top-level member) telling the handler where to POST the `{ protocol, data }` presentation result; the handler processes the request's `dcql_query`/`nonce`/`client_metadata` (and, when signed, `client_id` as audience), treats `response_mode: dc_api` and `expected_origins` as not applicable, and returns the result to `return_uri`. Relayed signed requests preserve Verifier audience binding (losing only origin pre-authorization); unsigned fall back to nonce-only binding.
