...
[[def: Agent]]:
~ The HTTP caller that requests a protected route, receives a x401 challenge, creates a wallet-facing OpenID4VP presentation request, receives proof material from a Wallet, and retries the protected route. The Agent uses an [[ref: Agent Identifier]] for client binding.
...
7. The [[ref: Agent]] retries the same protected route that produced the x401 challenge with either:
...
---- | ------- | ----------
Prefix | `x401` | REQUIRED. Domain-separates x401 challenges from other nonce or challenge values a Wallet may receive.
...
Nonce | `<nonce>` | REQUIRED. An opaque verifier-generated nonce segment encoded as base64url without padding. It MUST either identify stored challenge state or carry verifier-protected challenge state. It MUST contain or be bound to at least 128 bits of cryptographic randomness.
...
The `payload` parameter MUST contain the base64url-encoded [[ref: x401 Payload]]. A x401 challenge response SHOULD NOT require the Agent to parse a response body in order to understand the challenge.

The challenge response uses `WWW-Authenticate` because `Authorization` is an HTTP request header. The Agent uses `Authorization` only when retrying the protected route with a [[ref: VP Artifact]] or [[ref: Verification Token]].
...
`request_id` | OPTIONAL. A stable verifier-defined identifier for the proof template. This value can be reused across challenge instances and routes when they ask for the same proof requirement.
...
`expires_at` | REQUIRED. The time after which the Verifier will reject the challenge. The value MUST be an RFC 3339 timestamp.
...
A [[ref: Verification Token]] records the Verifier's decision that a presentation satisfied a x401 challenge. It is not a credential, payment artifact, or new issuer attestation about the credential subject.
